...
while tierno <= 4:
    ...
    print(
        "Please make sure you only condition on nodes of the previous tier. All possible parents are:"
    )
    ...
    while True:
        desc = input(
            "Specify one node at a time. Enter the label of the node and, after a hyphen, the labels of its parents, comma-delimited (e.g. Ian-Lara,Coleman), then hit ENTER.\n At most two parents are allowed for simplicity; all parents must be in the preceding tier.\n Hit ENTER without input when all nodes have been specified."
        )
        if len(desc) == 0:
            break
...
for parent in parents:
    freqs[parent] = input(
        "What is the frequency assigned to the parent node " + parent + " ?"
    )
